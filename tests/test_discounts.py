...
def test_premium_customer_with_few_orders():
    # A customer who IS premium but has < 5 orders
    # Correct rule: qualifies (True) - premium status alone qualifies
    # Buggy code with AND: does not qualify (False)
    customer = Customer(id="test_3", name="Test Premium Few Orders", is_premium=True, order_count=2)
    assert qualifies_for_free_shipping(customer) is True

def test_non_premium_customer_with_many_orders():
    # A customer who is NOT premium but has >= 5 orders
    # Correct rule: qualifies (True) - order count alone qualifies
    # Buggy code with AND: does not qualify (False)
    customer = Customer(id="test_4", name="Test Regular Many Orders", is_premium=False, order_count=7)
    assert qualifies_for_free_shipping(customer) is True
